...
- Each app installation or browser profile belongs to at most one device group. Version 4 authenticates its membership and key history as a signed transition chain anchored during pairing. Sessions carry a device-and-group-signed descriptor whose creator key must be an actual P-256 ECDH public key and whose signer must remain continuously in the group from that Session join onward; clients revalidate those conditions on synchronization and retire a locally stored session when either fails. Removing and later re-adding the signer does not restore that participation. This prevents the relay from keeping a removed device's Agent key as a future response destination. New responses use only the current usable key epoch. Adding a device creates a new transition; removing another device rotates the key immediately. A device leaving by itself signs only its removal marker, after which events pause until a remaining device verifies it and creates the fresh key. Removal cannot revoke ciphertext already received by that device.
